var MainCtrl = function ($scope, $timeout, $location, $anchorScroll) {

  $scope.viewManager = {};

  var path = $location.path();

  $scope.viewManager.showNews = (path.indexOf('anchor-news') > -1);

  if($scope.viewManager.showNews){
    var anchor = path.split("#")[1];
    $location.hash(anchor);
    $anchorScroll();
  }

  $scope.toggleNews = function(toggle) {
    $scope.viewManager.showNews = toggle;
    //if(!toggle) {
    presentation();
    //}
  };
...
angular.module('ebizApp').controller('MainCtrl', ['$scope', '$timeout', '$location', '$anchorScroll', MainCtrl]);